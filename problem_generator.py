...
def generate_solvable_puzzle(empty, full, size, colors):
    ...
    for _ in range(100):  # Shuffle 100 moves to create initial state
        ...
        if from_idx != to_idx and puzzle.can_move(from_idx, to_idx, num_colors):
            puzzle.move(from_idx, to_idx, num_colors)

    return puzzle.state

if __name__ == "__main__":
    initial_state = generate_solvable_puzzle(empty=1, full=4, size=4, colors=4)
    puzzle = LiquidPuzzle(empty=1, full=4, size=4, colors=4, initial_state=initial_state)
